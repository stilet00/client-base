...
const getPersonalPenalties = async (req: Request, res: Response) => {
	try {
		...
		const selectedTranslator = await Translator.findById(translatorId).populate(
			{
		...
		console.error("An error occurred while getting personal penalties:", error);
		res.status(500).send("An error occurred");
	}
};
...
const toggleSuspendClientResolver = async (req: Request, res: Response) => {
	try {
		...
		const errorMessage = "An error occurred while toggling client suspension";
