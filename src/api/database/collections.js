...
if (!process.env.DATABASE || !process.env.DATABASE_PASSWORD) {
    throw new Error('DATABASE and DATABASE_PASSWORD environment variables are required');
}
var DBConnectionCredentials = process.env.DATABASE.replace('<PASSWORD>', process.env.DATABASE_PASSWORD);
var changeDatabaseInConnectionString = require('./utils').changeDatabaseInConnectionString;
...
var _a = require('../models/translatorsDatabaseModels'), TranslatorSchema = _a.TranslatorSchema, BalanceDaySchema = _a.BalanceDaySchema;
...
var connectToDatabase = function () { return __awaiter(void 0, void 0, void 0, function () {
    ...
    return __generator(this, function (_a) {
        try {
            clientBaseDB = mongoose_1.default.createConnection(changeDatabaseInConnectionString(DBConnectionCredentials, 'clientBase'));
            Task = clientBaseDB.model('Task', TaskSchema, 'tasksCollection');
            Client = clientBaseDB.model('Client', ClientSchema, 'clientsCollection');
            Translator = clientBaseDB.model('Translator', TranslatorSchema, 'translatorCollection');
            Admin = clientBaseDB.model('Admin', AdminSchema, 'adminCollection');
            Statement = clientBaseDB.model('Statement', new mongoose_1.default.Schema(PaymentSchema, {
                collection: 'statementsCollection',
            }));
            BalanceDay = clientBaseDB.model('BalanceDay', BalanceDaySchema, 'balanceDayCollection');
...
(function (CollectionNames) {
    ...
    CollectionNames["CollectionTaskNotifications"] = "collectionTaskNotifications";
...
var getCollections = function () {
    return Object.values(CollectionNames).reduce(function (collectionsObject, collectionName) {
