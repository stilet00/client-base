	...
	const getStatementsGroupedByCommentAndYear = (statements) => {
		const groupedStatement = Object.values(FINANCE_COMMENTS).map((comment) => {
			...
			const groupedByCommentAndAmount = {
				comment: comment,
				amount: groupedByAmount.reduce((sum, current) => sum + current, 0),
			};
	...
	const monthProgress =
		monthTotalSum > previousMonthTotal ? (
			<span className={"green-text styled-text-numbers"}>
				...
				<CountUp
					duration={0.75}
					end={calculatePercentDifference(monthTotalSum, previousMonthTotal)}
				/>
				&nbsp;%
			</span>
		) : (
					...
					<Table aria-label="simple table">
						<TableHead>
							<StyledTableRow>
								...
								<StyledTableCell className="td-with-info">Data</StyledTableCell>
							</StyledTableRow>
						</TableHead>
						<TableBody>
									...
									<StyledTableRow>
										...
										<StyledTableCell className="td-with-info">
											...
											>
												<CountUp
													duration={0.75}
													end={monthTotalSum - svadbaMonthTotal}
													separator=" "
													prefix="$"
												/>
											</span>
							...
							{isAdmin && (
								<>
									...
									<StyledTableRow>
										<StyledTableCell>Salary payed</StyledTableCell>
										<StyledTableCell className="td-with-info">
											<span className={"blue-text styled-text-numbers"}>
												<CountUp
													duration={0.75}
													end={Math.floor(yearTotalSum * 0.45)}
													separator=" "
													prefix="$"
												/>
											</span>
									...
									{statementsGroupedByComment.map((statement) => (
										<StyledTableRow key={statement.amount + statement.comment}>
											<StyledTableCell>
												{statement.comment === "salary"
													? "Clients Salary"
													: statement.comment}
											</StyledTableCell>
											<StyledTableCell className="td-with-info">
									...
									<StyledTableRow>
										...
										<StyledTableCell className="td-with-info">
											<span
												className={"green-text styled-text-numbers"}
												style={{
													margin: 0,
												}}
											>
