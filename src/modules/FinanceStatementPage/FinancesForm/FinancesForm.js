				...
				<Fade in={open}>
					<div className={"form-container payment-form"}>
						<div className={"payment-form__header"}>
							<div className={"payment-form__header_picture"}>Compose bill</div>
						</div>
						<form className="payment-form__main">
							...
							>
								{listOfReceivers.map((receiver, index) => (
									<MenuItem key={receiver.id + index} value={receiver}>
										{receiver.label}
									</MenuItem>
								))}
							</TextField>
							<div className="payment-form__main_radio-container">
								<FormControl>
									...
									<RadioGroup defaultValue={FINANCE_SENDERS.agency}>
										{Object.values(FINANCE_SENDERS).map((sender) => (
											<FormControlLabel
												key={sender}
												value={sender}
												control={<Radio />}
								...
								<FormControl>
									...
									<RadioGroup defaultValue={FINANCE_COMMENTS.salary}>
										{Object.values(FINANCE_COMMENTS).map((comment) => (
											<FormControlLabel
												key={comment}
												value={comment}
												control={<Radio />}
							...
							<Button
								...
								disabled={
									paymentData.amount === 0 || arrayWithErrors.length !== 0
								}
								type={"button"}
